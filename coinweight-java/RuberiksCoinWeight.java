import java.util.*;
public class RuberiksCoinWeight {
  public int possibleValues(int weight, String[] coins) {
    ...
    for(int i=0; i<cache.length; i++) Arrays.fill(cache[i],false);
  ...
  void numvals(int valsofar, int wtleft, int[] val, int[] min, int[] max){
    ...
    if(cache[valsofar][wtleft]) return;
  ...
  public long longgcd(long a, long b){  //it's kind of creepy that people can watch me code.
    return (b==0)?a:longgcd(b,mod(a,b));
  }
  public long longlcm(long a, long b){  //somebody get my camera back from John.
